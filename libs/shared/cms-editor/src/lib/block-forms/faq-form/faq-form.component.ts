      ...
      <div formArrayName="items">
        @for (item of items.controls; track $index; let i = $index) {
          <fieldset class="cms-fieldset" [formGroupName]="i">
            ...
            <label class="cms-field"><span>Pregunta *</span><input formControlName="question" type="text" /></label>
            <label class="cms-field"><span>Respuesta *</span><textarea formControlName="answer" rows="3"></textarea></label>
            <button type="button" class="cms-btn cms-btn--danger" (click)="removeItem(i)">Eliminar</button>
          </fieldset>
        }
      </div>
      <button type="button" class="cms-btn cms-btn--secondary" (click)="addItem()">+ Añadir pregunta</button>
      <button type="submit" class="cms-btn cms-btn--primary" [disabled]="form.invalid">Aplicar</button>
    </form>
  `,
})
export class FaqFormComponent implements OnInit {
  readonly initialData = input<Partial<FaqData>>({});
  readonly changed = output<FaqData>();
  ...
  private buildItem(i?: { question?: string; answer?: string }): FormGroup {
  ...
  submit(): void {
    ...
    this.changed.emit({ title: v.title || undefined, items: v.items });
  }
}
